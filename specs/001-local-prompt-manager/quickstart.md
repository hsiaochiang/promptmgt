...
## 安裝與啟動
1) 安裝依賴：`npm install`。  
2) 設定根路徑：在 UI 設定頁輸入 `rootPath`，建議使用者目錄下隱藏資料夾（例：`%USERPROFILE%/.promptmgt` 或 `~/.promptmgt`）；若不存在應用會引導建立。  
3) 啟動開發伺服：`npm run dev` → http://localhost:3000。  
4) 首次載入確認 RootPathAlert 無錯誤；若路徑失效，依提示重新定位。
...
- 資料存放於使用者目錄隱藏資料夾，沿用 OS 權限，預設無應用層加密。

## 遙測與診斷
- 預設啟用本機匿名紀錄（不外傳），容量 5MB 環迴；可在設定停用或匯出診斷檔。  
- 記錄 CRUD、衝突對話框選擇、搜尋/插入耗時（僅本機檔案）。
