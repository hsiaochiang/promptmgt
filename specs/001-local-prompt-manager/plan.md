...
**Performance Goals**: 啟動/載入 50 專案 + 500 提示詞 ≤5 秒；搜尋 p95 ≤2 秒（1000 筆內，>1000 截斷為 1000 並提示截斷旗標）；自動儲存持續 2 秒節奏；Pin 收合動畫 150–250ms；精簡複製 驗收 ≤3 秒（內部目標 <150ms）；草稿→轉正流程 ≤30 秒；首次導覽成功率（SC-009） ≥80%；收件匣 >100 筆啟用分頁（預設 50/頁，可配置，搜尋需跨頁）；片語 CRUD/UI/API 驗收以 FR-010 為準，FR-021 僅保留追蹤；UX 檢核新增 SC-016~SC-020（Inbox 淨空、Undo/Redo 易發現性、Accordion 快捷鍵、錯誤敘述、長操作忙碌指示）
...
- UX SLA：依 docs/ux-checks.md 驗證 SC-009、SC-010 及新增 SC-016~SC-020（Inbox 淨空、Undo/Redo 易發現性、Accordion 快捷鍵、錯誤敘述、長操作忙碌指示），並納入 T062/T063 對應表。
