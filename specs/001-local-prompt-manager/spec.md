...
- Snackbar Undo：刪除後 5 秒可復原（復原/關閉）
...
- 行為：刪除類操作均需 Confirm，刪除後顯示 Snackbar Undo（5 秒）
...
- **Markdown 編輯與預覽**：提供編輯器與預覽，支援語法高亮與常用區塊（程式碼、列表、標題）。
...
- 當 rootPath 缺失或無法存取時，RootPathAlert 需提供導向設定頁的行動入口。
...
4. **Given** 使用者刪除專案/提示詞/剪貼簿項目，**When** 確認刪除，**Then** 顯示 Snackbar Undo（5 秒）且可在時限內復原
