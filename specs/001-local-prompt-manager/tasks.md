...
# 任務清單：原型對齊提示詞工作台（001-local-prompt-manager）
...
- 每行 task 必須「行首直接以 `- [` 開頭」（不允許前置空白/縮排）
- 勾選狀態：`- [ ]` = 未完成；`- [x]`/`- [X]` = 已完成（大小寫皆可）
...
**Checkpoint**：共用基礎完成（已具備 taxonomy/schema/error/UI primitives）。


---

## 階段 3：使用者故事 1 - Workspace Shell + Topbar Tabs（Priority: P1）
...
## 階段 4：使用者故事 2 - Projects + Project Detail + README（Priority: P1）
...
## 階段 5：使用者故事 3 - Prompts（列表）（Priority: P1）
...
## 階段 6：使用者故事 4 - Prompt Detail（編輯/預覽 + autosave + 複製 + 409 三選一）（Priority: P1）
...
## 階段 7：使用者故事 5 - Scratchpad（剪貼簿）（Priority: P1）
...
## 階段 8：使用者故事 6 - 刪除 + Undo（5 秒 deferred delete）（Priority: P1）
...
- [X] T069 [P] 片語庫 UI（清單/搜尋/插入/CRUD/Topbar 切換）：app/(workspace)/components/snippet-panel.tsx, app/(workspace)/hooks/useSnippetInsert.ts, app/(workspace)/components/top-bar.tsx
