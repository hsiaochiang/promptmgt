...
- [ ] T053 [P] 收件匣 100+ 分頁/搜尋（Edge Case）：草稿超過 100 筆時啟用分頁（預設 50/頁，可配置，與 spec 對齊），搜尋需跨頁；顯示整理提示文案。
- [ ] T053a [P] 收件匣分頁/搜尋測試：整合測試驗證僅顯示第一頁、跨頁搜尋命中後頁、空態/提示文案；覆核分頁大小預設 50/頁與可配置行為。
...
- [ ] T055 [P] T045a 效能量測腳本：建立啟動/搜尋/精簡複製/Pin 動畫/autosave 的量測腳本或可重現手動步驟，與目標數據對齊。
- [ ] T056 [P] T045b 效能實測與記錄：在 docs/perf-checks.md 填入實測數據與偏差，未達標列調優方案與負責人。
- [ ] T057 [P] T045c 回歸檢查掛鉤：將搜尋截斷、Pin 動畫、autosave 節流的量測納入回歸（CI 或手動清單），描述執行頻率與觸發條件。
- [ ] T058 [P] T047a 覆蓋率掃描：執行 `vitest --coverage` 產生 lcov，列出覆蓋 <80% 或關鍵路徑 <100% 清單（含檔案/行號）。
- [ ] T059 [P] T047b 覆蓋率補測：針對 conflict handling、root path init、search truncation、batch ops、undo/snackbar 等補單元/整合測試並更新 lcov。
- [ ] T060 [P] T047c 覆蓋率流程文件：記錄覆蓋率檢查流程與閾值（≥80%，關鍵 100%），標明 CI/手動檢查步驟與失敗處理；連結到 coverage/lcov-report/index.html。
