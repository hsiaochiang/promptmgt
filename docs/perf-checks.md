...
3) **複製耗時（SC-003）**：於提示詞頁點「複製完整」與「複製精簡」，量測點擊到剪貼簿完成時間，驗收 ≤3s、內部目標 <150ms，留下 trace（trace-003.json）。  
4) **搜尋 p95（SC-004）**：在 1000+ 筆資料下輸入常見關鍵字，量測鍵入→渲染時間並確認截斷旗標，記錄 p95（trace-004.json）。  
5) **片語插入與 Pin 動畫（SC-007/SC-011）**：開啟 Drawer 搜尋/插入、切換 Pin 觸發收合，各自量測總耗時/動畫時間並保存 trace。  
6) **autosave 節奏**：開啟草稿/提示詞，修改後觀察 2 秒自動儲存節奏與 409 提示，記錄是否符合節流設定。  
7) **填表/存證**：將各步驟數據回填下方表格與 SC 對應表，並附上 trace/.har 位置（儲存在 repo 根目錄 trace/ 或 PR artifact）。  
